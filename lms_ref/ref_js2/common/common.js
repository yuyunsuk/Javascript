		...
		$(document).ajaxComplete(function( event, xhr, settings ) {
			...
			if (resultStr != undefined && resultStr.length == 2) {
				...
				if (exceptionMsg == 'ERROR-SSO-NEED-TO-JOIN') {
					...
					if(fullUrl.includes("/usrs/clssrm/contentsStudyPpForm.do")) {
						window.close();
					}
				}
...
/**
    ...
    function checkJuminNo(juminNo, juminNo1, juminNo2, nextFocus)
    {
        ...
        var juminNoElm  = (form.elements[juminNo]);
        var juminNo1Elm     = (form.elements[juminNo1]);
        var juminNo2Elm     = (form.elements[juminNo2]);
        var nextFocusElm    = (form.elements[nextFocus]);
...
//            domArr[domArr.length-1].search(knownDomsPat)==-1) {
    ...
    function validateObj(form, obj) {
        ...
        if(lenCheck != null)
        {
          if(jsByteLength(value) > lenCheck)
            {
        ...
        if(byteCheck != null)
        {
          if(jsByteLength(value) > byteCheck)
            {
    ...
    function jsRateCalc(currencyCd, yRate, term) {
        ...
        var rate = ((yRate / 100) * (term / 12) * (365 / yday));
  ...
  function fixScreen(){
    ...
    */
    ...
    function getIntervalMonth(kijunDate, term, isPrevNext)
    {
        ...
        {
            addMonth    = month - term;
            addYear     = Math.floor(addMonth/12); /// 빼줄 년도 계산
            tempYear    = (addYear) + (addMonth%12);
            if(tempYear > 0)
            {
                tempMonth   = (tempYear%13);
            }
            else
            {
                tempMonth   = (12 + addMonth%12);
                if(tempYear == 0)
                    addYear     = addYear-1;
            }
        }
        ...
        {
            addMonth    = month + term;
            addYear     = Math.floor((addMonth/13)); /// 더해줄 년도 계산
            tempYear    = addYear + (addMonth%13);
        ...
        if( !isValidDay((year) + (addYear), tempMonth, date))
            date = getLastDay((year) + (addYear), tempMonth);

        rtnDate = (year) + (addYear) +""+ tempMonth +""+ date;
        //alert(">날짜 ::"+ rtnDate);
   ...
   **/
    ...
    **/
	...
	function getObject(objName) {
		var returnObj = null;
		if (browserType == "IE") {
			returnObj = ("document.all."+objName);
		}
		else if (browserType == "NE") {
